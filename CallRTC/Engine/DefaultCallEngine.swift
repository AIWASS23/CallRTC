...
class DefaultCallEngine: NSObject, CallEngine {
    ...
    func startCall(_ configuration: CallConfiguration) {
        let dto = StartCallRequest(
            calleeId: configuration.calleeId,
...
extension DefaultCallEngine {
    ...
    func sendReadyToOffer() {
        guard let call = call else { return }
        
        let payload = ReadyToOfferPayload(
            callId: call.callId,
    ...
    func sendEndCall(reason: EndCallReason) {
        guard let call = call else { return }
    ...
    func sendIceCandidate(candidate: RTCIceCandidate) {
        ...
        let iceCandidatePayload = IceCandidatePayload(
            senderId: call.senderId(),
            receiverId: call.receiverId(),
    ...
    func handleAnswer(message: SocketMessage<AnyCodable>) {
    ...
    func handleOffer(message: SocketMessage<AnyCodable>) {
    ...
    func handleEndCall(message: SocketMessage<AnyCodable>) {
        call = nil
        webRTCManager?.terminate()
        webRTCManager = nil
    ...
    func handleIceCandidate(message: SocketMessage<AnyCodable>) {
    ...
    func handleError(message: SocketMessage<AnyCodable>) {}
    
    func handlePing(message: SocketMessage<AnyCodable>) {
